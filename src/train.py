...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            if global_step % sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    zs_samples = zs[:8]
                    zs_samples = torch.nn.functional.normalize(zs_samples, p=2, dim=1)
                    ...
                    data_radius = 27.7
                    samples = samples * data_radius
                    samples = rae.decode(samples)
                    
                    # Save...
                    from torchvision.utils import save_image
                    path_save_sample = f"{experiment_dir}/sample/"
                    if not os.path.exists(path_save_sample):
                        os.makedirs(path_save_sample, exist_ok=True)
                    
                    save_image(
                        samples,
                        f"{path_save_sample}/samples_step_{global_step:07d}.png",
                        # nrow=4, 
                        # normalize=True, 
                        # value_range=(-1, 1) # Depends on your RAE
                    )
        ...
        if rank == 0 and num_batches > 0:
            ...
            logger.info(
                f"[Epoch {epoch}] "
                + ", ".join(f"{k}: {v:.7f}" for k, v in epoch_stats.items())
            )
            if args.wandb:
