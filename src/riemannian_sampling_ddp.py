...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    ...
    for i in tqdm(range(num), desc="Building .npz file from samples"):
        sample_path = f"{sample_dir}/{i:06d}.png"
...
def main(args):
    ...
    target_radius = args.default_radius

    
    if rank == 0:
        print(f"Using Sampling Radius: {target_radius:.4f}")
    # ----------------------------------
...
if __name__ == "__main__":
    ...
    parser.add_argument("--default-radius", type=float, default=45.10, 
                        help="Fallback radius if no reference path is provided.")
    parser.add_argument("--reference-path", type=str, default=None,
                        help="Path to a reference image folder (e.g. val set) to compute dynamic radius.")
